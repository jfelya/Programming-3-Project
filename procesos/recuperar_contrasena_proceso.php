...
if (isset($_REQUEST["con"]))
	{
		switch ($_REQUEST["con"])
		{
			...
			else
			{
				...
				mysqli_close($conexion);
				header("Location:../recuperar_contrasena.php?id_cliente=$id_cliente");
			}
			break;
			...
			if ($respuesta_secreta !== $row["respuesta_secreta"])
			{
				...
				mysqli_close($conexion);
				header("Location:../recuperar_contrasena.php?id_cliente=$id_cliente&error=1");
			}
			else
			{
				$_SESSION["cambiar_contrasena"] = TRUE;
				mysqli_close($conexion);
				header("Location:../recuperar_contrasena2.php?id_cliente=$id_cliente");
			}
			break;
		}
	} // SI ESTÁ PUESTO "CON"
