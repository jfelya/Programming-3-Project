...
$sql_prueba = "SELECT * FROM usuarios WHERE id_cliente = '$id_cliente'";
...
{	
	if (strlen($contrasena) < 2 || strlen($contrasena2) < 2)
	{
		$_SESSION["mensaje"] .= "Las contraseñas no pueden tener menos de 2 caracteres. ";
		$todo_bien = FALSE;
	}
	if (strlen($contrasena) > 20 || strlen($contrasena2) > 20)
	{
	...
	elseif ($contrasena != $row_prueba["contrasena"] || $contrasena2 != $row_prueba["contrasena"]) {
	...
	if ($todo_bien == FALSE)
	{
		...
		header("Location:../recuperar_contrasena2.php?id_cliente=$id_cliente&error2=1");
			...
			{
				$sql = "UPDATE usuarios SET contrasena = '$contrasena' WHERE id_cliente = '$id_cliente'";
